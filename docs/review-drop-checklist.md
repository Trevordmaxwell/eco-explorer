...
For an alpha release candidate, run the strict wrapper:

```bash
npm run alpha:rc
```

For a manual source review drop:
...
- Local source-control and scratch state such as `.git/`, `.tmp/`, `.DS_Store`, `._*`, `__MACOSX/`, editor folders, logs, and `work-queue.additions-130-157.md`.
- Any generated review archive or extracted review workspace.

## Clean Extract Proof

`npm run review:verify -- <archive>` extracts the archive into `.tmp/review-drop-verify/`, checks the forbidden paths before installing, runs `npm ci`, then runs `npm run validate:agents`, `npm run science:check`, `npm test`, and `npm run build`. `npm run alpha:rc` wraps that clean-extract proof after first validating, testing, and building the source workspace.
