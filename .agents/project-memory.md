...
- The Route-v2 support slot should stay tiny and reuse existing seams only: the live set is now `hand-lens`, text-only `note-tabs`, and owned `route-marker` rather than another marker, prompt overlay, inventory row, or unlock tree.
- The live support-slot behavior should persist the chosen `selectedOutingSupportId` in save state, with `route-marker` driving the world-map marker only when explicitly selected, `hand-lens` keeping the compact `TODAY` strip clue-first plus inspect-bubble notebook-fit help, and `note-tabs` keeping the strip notebook-first through route-summary and filing-preview copy.
- The notebook return payoff should stay route-authored and one-press: `NOTEBOOK READY` now uses per-route filing language, and filed Route v2 notices should surface the compact authored note result instead of a title-only toast.
...
- Lane 1's next queued wave is `High Pass` board activation plus archived-season-to-active-season continuity polish through the existing station and map seams.
- Lane 2's next queued wave is canopy-and-cavern content depth plus one compact memory payoff for the newer vertical spaces.
...
- Lane 3's next queued wave is a stronger forest giant-tree destination loop followed by the first compact treeline lee-side ascent proof.
...
- Lane 4's next queued wave is the first process-backed Route v2 outing followed by one slightly richer notebook filing return, still without opening a larger station shell, quest ledger, or inventory system.
...
- The first second-season opening pass is now live through those same seams: once the season is filed, the station should default to `SEASON -> EXPEDITION`, the logged `ROOT HOLLOW` card should hand off straight into a `Treeline Pass` / `High Pass` world-map focus, and `Route Marker` should point at that same node instead of falling silent.
