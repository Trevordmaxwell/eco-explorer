...
- The next highest-leverage gameplay wave should be living-world route differentiation in a lighter-weight biome band, preferably `Tundra` first and `Coastal Scrub` second, where support choice plus active world-state changes how the player finds, reads, or approaches a clue in play.
- Do not spend the immediate next wave on more beach opener or treeline signature density; those bands are already near their comfortable handheld ceiling and should only reopen for later contrast or regression work.
