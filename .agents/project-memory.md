...
- When helper differentiation needs a stronger in-field proof, prefer route-fit clue targeting inside existing inspect seams over more season-strip copy; the first live example is `hand-lens` preferring the nearest in-range notebook-fit clue while other supports keep the normal nearest inspectable.
