...
- The support-readable follow-on is now live on the existing top-right `NOTEBOOK J` chip too: when `hand-lens` is currently selecting a live preferred route clue, the chip may show one tiny support-biased badge state while leaving the route title, default hint behavior, and non-`hand-lens` supports unchanged.
