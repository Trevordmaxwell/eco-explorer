...
Eco Explorer is a desktop-first retro-style educational exploration game for kids. The current live build includes a completed five-biome alpha arc from beach through filed `High Pass`, with world-map travel, inspectable nature objects, field routes, a field station, collectible discoveries, and a persistent journal.
...
- Same-pane comparison cards remain viable at the current handheld size, but their note summaries are already near the comfortable copy ceiling; future comparison follow-ons should keep summaries compact unless the card layout changes.
- When a shared entry can unlock multiple valid local ecosystem notes, future comparison follow-ons should prefer a tiny comparison-side preferred-note mapping instead of reordering the general ecosystem-note resolver that powers the rest of the notebook.
...
- The alpine comparison lattice is now strong enough that later treeline and tundra richness passes should prefer local landmark close-look or sketchbook support over more shared-species comparison growth, unless a clearly new contrast appears.
- The second-act local-support ceiling is now spread across two compact lane-2 passes: `talus-cushion-pocket` + `frost-heave-hummock`, then `krummholz-spruce` + `woolly-lousewort`; later treeline and tundra follow-ons should treat the current close-look and sketchbook-strip lengths as the notebook-facing ceiling and switch carriers or surfaces before stacking more copy into those same compact seams.
...
- The existing `OUTING SUPPORT` notice may be route-aware: active gathering routes can name the selected support's immediate route effect, while ready-to-file routes should stay calm and avoid implying a map marker or in-field hunt.
- The live support-slot behavior should persist the chosen `selectedOutingSupportId` in save state, with `route-marker` driving the world-map marker and snapping world-map focus to the active outing target only when explicitly selected, `hand-lens` keeping the compact `TODAY` strip clue-first plus inspect-bubble notebook-fit help, `note-tabs` keeping the strip notebook-first through route-summary and filing-preview copy, and `place-tab` unlocking once `treeline-stone-shelter` is logged so it can keep the strip question-first through the active `tundra-short-season` thaw-edge prompt, the inland `tundra-survey` / `Bright Survey` `short-summer-rush` question, plus the authored `edge-pattern-line` prompts.
...
- Route catalog authoring now has a focused guard test that keeps canonical filed text, display-only replay prefixes, exact evidence-slot ordering, clue-backed filing tails, and active alternate clue references from drifting as new Route v2 definitions are added.
...
- Forest Trail now has a broader layered traversal spread across fern shade, Creek Bend cool cover, and the lower giant-tree foot pocket; future forest density should build from these three remembered bands instead of piling more labels into the already-dense root-hollow or upper-canopy pockets.
...
- Guided world-map focus should prefer explicit `Route Marker` support when active, then fall back to `GuidedFieldSeasonState.nextBiomeId` before the current-location fallback so early `M` + `Enter` travel points at the taught next habitat without adding tutorial UI.
...
- Coastal Scrub now has a stronger forest-edge release pocket after the shore-pine rest, using a tiny root-lip / berry-rest path plus authored cover carriers (`nootka-rose`, `deer-mouse`, `salmonberry`, `song-sparrow`, `sword-fern`, and `nurse-log`); future scrub traversal should treat this as the remembered far-side transition before adding more swale density.
- Beach now has a tiny wrack pocket between the lee drift and tidepool approach, using low driftwood plus authored `bull-kelp-wrack`, `beach-hopper`, `pacific-sand-crab`, and `western-snowy-plover`; future beach traversal should avoid crowding the tide line and instead build from this wrack-food-line beat if it needs more motion.
- Treeline and Tundra now have second-pass high-country pocket density: Treeline's post-Stone-Shelter dwarf-shrub pocket should read as wind shelter and low woody cover, while Tundra's snow-edge pocket should stay low, thaw-adjacent, and animal-readable rather than becoming another climb-focused space.
...
- The next director-approved phase is the `Steady Beta Foundation` push: surface triage, a dedicated Source to Shore station container, route-flow consolidation, route catalog extraction, one narrow game-coordinator split, compact filed-memory payoff, spatial playthrough polish, and full-arc signoff.
- Source to Shore should get its own compact station/board identity instead of continuing to ride inside the completed `EDGE LINE LOGGED` first-season board.
- The Source to Shore station container is now live as a dedicated `source-to-shore-beta` route board with exactly three visible beats (`source-shelter`, `forest-release`, `dune-catch`); it keeps the first-season archive strip, atlas logged-route history, and nursery capstone hint intact rather than using a launch card inside `EDGE LINE LOGGED`.
- Source to Shore active/ready/filed behavior now resolves through the explicit `SOURCE_TO_SHORE_BEAT_FLOW` matrix in `src/engine/source-to-shore-state.ts`; the station board consumes that beat surface state, and ready-to-file states must stay map-calm with no route-marker target or replay label.
- Source to Shore now has active-only replay names for all three live beats: `Rime Source`, `Cool Release`, and `Held Dune`; the canonical ready/filed identities remain `Source Shelter`, `Forest Release`, and `Dune Catch`, with no fourth beat.
- Do not append a new playable Route v2 request after filed `source-to-shore-dune-catch` until a station route-family boundary is explicitly scoped; otherwise map/journal active outing state can outrun the filed Source to Shore board, atlas, archive, and support expectations.
- Filed Source to Shore terminal station copy now resolves through `resolveSourceToShoreFiledArcCopy()`; after `Dune Catch` is filed, routes and expedition subtitles should read as current-field-arc closure instead of reopening the older `High Pass opens the next field season` subtitle.
- Route/request authoring data now lives in `src/engine/field-request-catalog.ts`; keep active request resolution, support behavior, evidence progression, filed-note synthesis, and notebook filing in `src/engine/field-requests.ts`.
- Field-station session selection rules now live in `src/engine/field-station-session.ts`; keep station surface cycling, route/support row selection, nursery-card selection, and selection normalization there while `src/engine/game.ts` keeps input polling and side effects such as filing, purchases, support toggles, notices, saves, audio, expedition activation, and nursery actions.
- Field-station primary-action intent also lives in `src/engine/field-station-session.ts`; it may classify file-note, support, purchase, expedition, and nursery intents, but `src/engine/game.ts` should continue to perform the side effects, notices, audio, persistence, and debug-state updates.
- World-map travel label policy now lives in `src/engine/travel-framing.ts`; keep origin labels, map-return labels, focused map summaries, walking approach labels, and small regional travel warmth there instead of rebuilding those rules inside `src/engine/game.ts`.
- Even after clean full-arc proof, do not add a fourth Source to Shore beat in the current runway; hold the three-beat endpoint unless a later playtest or explicit director override changes the direction.
- The next director-approved growth shape is the `Parallel Beta Runway`: lane 2 content richness, lane 3 spatial depth, and lane 4 route-feel work may run independently without waiting on lane 1, while lane 1 first closes the remaining coordinator-split and full-arc signoff tail before starting new systems growth.
- Future large pushes should minimize cross-lane gates by giving each lane its own packet runway, focused file family, and handoff notes instead of making content, spatial, and route work wait on station or systems implementation unless there is a real shared-object dependency.
- The next director-approved phase is the `Beta Expansion` push: twelve packets split into three per lane, with lane 1 station/travel/proof work, lane 2 front-half/forest/high-country content, lane 3 coastal/forest/high-country spatial depth, lane 4 route design/replay/filing synthesis, and only the final integration signoff crossing lanes.
- For this Beta Expansion push, start one lane runner per lane from `scout-487`, `scout-490`, `scout-493`, and `scout-496`; downstream work should stay lane-local until the final signoff packet.
...
- Route-aware living-world passes should prefer one compact active-request world-state seam so the active request title, enter-biome notice, board replay note, and `TODAY` wrap can align during the live window while notebook-ready and filed identities stay canonical.
- When a live world-state window needs to change route feel in practice, prefer one route-local evidence-slot opportunity seam tied to already-authored ecology over a bigger replay system, planner layer, or hard gate.
- That route-local replay seam now supports alternate clue carriers from both `processFocus` and `worldStateFocus`; the first live early/inland examples are `Moist Hollow` on `forest-moisture-holders` and `Brief Bloom` on `treeline-low-fell`, while notebook-ready and filed route identities stay canonical.
- When helper differentiation needs a stronger in-field proof, prefer route-fit clue targeting inside existing inspect seams over more season-strip copy; the live rule is that `hand-lens` may prefer active process-backed alternate carriers for the current next route slot, while other supports keep the normal nearest inspectable.
...
- Sprint 1's next Coastal Scrub memorability pass should complete one bluff -> swale -> forest-edge visual trio through the existing `pacific-wax-myrtle` bluff anchor plus new `beach-strawberry` and `salmonberry` close-look cards, rather than spending more notebook prose or new comparison structure.
- Sprint 2's first lane-2 relationship-teaching coast pass should stay compact and late-beach-first: use the existing tide-line note and notebook-prompt surfaces to teach `wrack -> tiny scavengers / sand crabs -> birds`, rather than opening another comparison branch or a bigger journal shell.
- Sprint 3's first lane-2 chapter science pack should stay chapter-facing and `Treeline Pass`-owned: strengthen the live `High Pass` opener through current `Stone Shelter` and `Low Fell` carriers such as `hoary-marmot` and `dwarf-birch`, rather than reopening a broader alpine or tundra richness wave.
...
- The first support-shaped route-feel proof is now live on `Thaw Window` and the second reuse proof is now live on `Held Sand`: `hand-lens` can bias the live inspect target toward the authored notebook-fit carrier (`woolly-lousewort` in tundra, `beach-grass` in Coastal Scrub) while non-`hand-lens` supports do not. The next wave should make that difference easier to feel at a glance with one tiny cue, not more route text or more geometry.
- That first support-readable cue should land in the inspect bubble's existing accent line, using a stronger one-line label only for active hand-lens wins instead of widening the season strip, route card, or notebook shell.
- The support-readable follow-on is now live on the existing top-right `NOTEBOOK J` chip too: when `hand-lens` is changing what `E` will inspect right now, the chip may show one tiny support-biased badge state while leaving the route title, default hint behavior, and non-`hand-lens` supports unchanged. That broader chip rule covers both ordinary notebook-fit retargets and the narrower preferred active-clue alternates.
- The next route-feel follow-on should also keep buying down coordinator risk: support-biased inspect-target selection and its debug export should keep moving out of `game.ts` into the dedicated request/controller seam instead of growing a second wrapper cluster in the coordinator.
...
- The nursery and home loop are now back in regression-only territory unless a fresh handheld overlap or payoff failure appears; the next immediate gain is route-feel readability and maintainability, not more nursery surgery.
...
- The 2026-05-14 director reset made `main` the only local and remote branch line, committed the beta expansion baseline, and kept packet `182` as the active gate before new lane-parallel breadth.
- The next director sprint should treat native `256x160` browser readability as a hard playability gate: filed Source to Shore station overlap, cramped title guidance, first-play task truncation, and Source Shelter journal route-card clipping must be fixed or explicitly cleared before opening broader content, vertical, or route expansion.
- Fresh lane agents should read `.agents/fresh-lane-start.md` after `AGENTS.md`; it stores the current director gate, active lane copy-paste prompts, retired-lane redirects, and the reasoning-effort defaults for the current two-lane model.
- As of the 2026-05-14 director update, the active operating model is director plus two lanes: lane 1 owns playability, systems, station, routes, support, replay, and integration proof; lane 2 owns world richness, science content, atlas/journal payoff, close-look, sketchbook, spatial readability, vertical traversal, and sub-ecosystem depth. Former lane 3 work routes through lane 2, and former lane 4 work routes through lane 1.
- After packet `192`, the next active work is packet `193` RC/playtest readiness: prove the current build through an internal player-facing smoke pass and refresh observer playtest docs before any new route, content breadth, station page, save schema, traversal system, planner, or feature expansion starts.
...
- The next-chapter shell follow-on is now live as a filed-season expedition pivot: once `Season Threads` is filed, the expedition tab stops spending its footer on `NEXT FIELD SEASON` and instead becomes one real `HIGH PASS / NEXT` card. Future chapter surfacing should keep growing inside that expedition-card helper instead of adding another station page or recap footer layer.
- Station homecoming is now a station-owned seam: `arrivalMode: homecoming` may resolve one transient `WELCOME BACK` line from completed filed-progress milestones, expose `fieldStation.homecoming` in debug state, and drive one tiny upper-frame `hasHomecomingFrameAccent` cap through `hasHomecomingMemory` / `homecomingMilestoneRequestId`; future lanes should consume this seam instead of inventing parallel homecoming state or adding more lower-sill, side-gutter, lintel, panel, or copy weight.
...
- The late-season homecoming shell follow-on is now live as one dedicated `field-station-homecoming-shell.ts` seam: early and mid station returns keep the prior brace and sill behavior, while the archived `SEASON ARCHIVE` `High Pass` return adds one tiny upper lintel on that same brace family instead of spending more copy or station chrome.
- Sprint 4's first lane-2 High Pass follow-on should stay relationship-first and chapter-owned: spend it on one compact `lichen-fell` note around `moss-campion`, `reindeer-lichen`, and `talus-cushion-pocket` so the chapter teaches `rime + foothold` ecology, rather than making the new `hoary-marmot` or `dwarf-birch` close-look cards denser.
- Post-season chapter routes should respect that same calm close: the next route may unlock from the filed capstone, but it should stay dormant until `seasonCloseReturnPending` clears instead of overriding the return-to-station beat. The first live example is `High Pass`, which then takes over as a Route v2 outing through `stone-lift -> lee-watch -> rime-mark -> talus-hold` in `Treeline Pass`, with `frost-rime` able to locally reframe it as `Rimed Pass`.
...
- Filed-season `High Pass` chapter composition should now grow through `src/engine/high-pass-chapter-state.ts`: outing locator, routes continuity copy, dormant/live atlas note, routes launch card, and the `HIGH PASS / NEXT` expedition card should stay derived from that helper instead of regrowing inline inside `field-season-board.ts`.
...
- The first route-local live-opportunity follow-on is now live on that same beach route: during active `wrack-hold`, `beach-hopper` may also satisfy the final `wrack-line` stage on `beach-shore-shelter`, so the `Wrack Shelter` window changes actual tide-line opportunity while notebook-ready and filed states stay canonically `Shore Shelter`.
- When a Route v2 slot names a required zone, both hand-lens preview and route advancement should validate the inspected entity's own zone as well as the player's current zone, so cross-boundary inspect range cannot satisfy the wrong stage.
- The next coastal replay-consequence follow-on is now live as one in-place `scrub-edge-pattern` process variant: during the late `sand-capture` window, the active outing reframes as `Held Sand` across the request, replay note, and season-wrap seams, while notebook-ready and filed states stay canonically `Scrub Pattern` and the lighter `Haze Edge` / `Pioneer Clue` cues remain board-only.
- The Held Sand support-consequence follow-on is now live as one `note-tabs` replay exception on that same `scrub-edge-pattern` window: during active `Held Sand`, `hand-lens` keeps the live process sentence while `note-tabs` may instead hold the stable `SCRUB PATTERN` route line, so the support row distinguishes the moment-specific read from the notebook route without widening the replay system.
- The second route-local live-opportunity proof is now live as one same-zone replay carrier swap on `scrub-edge-pattern`: during active `Held Sand`, `beach-grass` may also satisfy the opening `open-pioneer` stage, and future route-opportunity follow-ons should prefer this existing-stage plus same-zone process-carrier shape before inventing broader replay-only branches.
- The first lane-2 light-band route-support follow-on should favor Tundra's `Thaw Window` band, using same-band `arctic-willow`, `bigelows-sedge`, and `tussock-thaw-channel` carriers in `thaw-skirt` before adding more density to Coastal Scrub; `Held Sand` already carries the scrub-side proof.
- Lane 1 now protects route-differentiation growth through `src/engine/field-request-controller.ts`: route-state reads, hand-lens notebook-fit gating, and outing-support notice copy should land in that helper seam before adding more route-facing conditionals back into `src/engine/game.ts`.
- Lane 1 should now keep support-biased inspect targeting in that same controller seam too: nearest inspectable fallback, hand-lens-preferred route targeting, and the paired `nearestInspectableEntityId` debug projection should extend `src/engine/field-request-controller.ts` instead of regrowing inside `src/engine/game.ts`.
...
- The next growth wave should prioritize tactile feel, visible payoff, and legibility inside the current five-biome world over adding biome 6, a bigger quest log, crafting, combat, or inventory sprawl.
- Living-world state should increasingly matter to play, not just flavor: future route work should use day-part, weather, phenology, and process moments as soft opportunity layers that change clue reading, route emphasis, or replay value.
- Each biome should earn at least one unforgettable signature pocket or route beat that players remember by feel, not only by note title; `Coastal Scrub` currently has the greatest need for stronger emotional and spatial identity.
- The field station and nursery should show player progress more visibly over time, with stronger home-place evolution and return payoff instead of only deeper logic.
- The first visible home-place pass is now live inside the nursery cards themselves: compost utility, unlocked extras, and teaching-bed growth should read as tiny card-local accents and footer motifs before the player reads more copy, and mature-bed states should avoid repeating the same route clue in multiple text rows.
- The next visible home-place pass is now live on the shared station shell: the lower sill remains the calm transformation seam, with nursery-driven planter growth in the middle and route-earned threshold accents on the sides, so `SEASON` and `NURSERY` both read as a more settled home place without another text row or decorative band.
- The next home-place follow-on is now live on a different compact seam: field-station side-gutter backdrop braces carry the non-sill payoff family, while the lower sill stays the capped threshold seam instead of taking more ornament.
- The first overlay-render protection split is now live as a render-only sibling seam: the field-station `SEASON -> ROUTES` page body lives in `src/engine/field-station-routes-page.ts`, while `src/engine/overlay-render.ts` keeps shell framing, page switching, expedition/nursery branching, and the shared accent passes.
- The nursery and teaching-bed page are now the weakest handheld surface in the game; the next home-loop wave should prioritize legibility and payoff there before another beach or treeline destination push.
- Nursery pages should hard-cap copy by state, guarantee zero overlap at `256x160`, and prefer dedicated renderer or layout seams over stacking more logic back into `overlay-render.ts`.
- Nursery communication should separate plant state, ecological reflection, and utility or reward into distinct beats instead of trying to explain all three inside one crowded card.
- When a nursery `TEACHING BED` is active and selected, the bed should own that home-loop moment; route-support guidance should yield until focus moves back to `bench` or `compost` instead of competing on the same card.
- The first nursery copy-budget pass is now live as an authored state-beat model: inactive selected beds use one `summary`, active beds use one `stageSummaryByStage[...]` sentence, mature beds keep the optional `memorySummary` footer, and route-support `rewardSummary` language no longer doubles as selected teaching-bed body copy.
- The nursery legibility wave is now live as a dedicated `src/engine/field-station-nursery-page.ts` seam with state-aware bed bands; follow-on nursery work should prefer lane 2's authored copy-role tuning before another lane-1 structural layout pass unless a new `256x160` overlap bug is proven.
- The first nursery reward-surfacing follow-on is now live as one selected `COMPOST HEAP` notice exception: once crowberry utility is unlocked, the compost notice should prefer the authored `Cool Heap Cover` line so utility payoff lands outside the teaching-bed card instead of widening the nursery page.
- Nursery unlock summaries should follow the current live route-facing title only when an older label has been fully retired; keep explicit dual-title exceptions like `Short Season` vs `Thaw Window` stable until a dedicated naming pass says otherwise.
- The nursery and home loop now read as intentional designed surfaces; future nursery work should default to regression-only follow-ons unless a new handheld break appears, not another broad surgery pass.
- The project is near its comfortable text ceiling at `256x160`, so future depth should prefer spatial design, motion, feedback, and environmental reaction over adding more copy.
- Before any season-three-scale expansion, lane 1 should spend explicit effort splitting responsibility out of `src/engine/game.ts`, `src/engine/overlay-render.ts`, and `src/engine/field-season-board.ts` so later chapter growth does not turn every change into surgery.
...
- Route-complete feel now rides the shared field-notice seam with two compact variants only: `notebook-ready` and `filed-route`. Future feel passes should reuse that badge-and-pulse language instead of widening the notice copy or adding another station surface.
- The first controller-protection follow-on for that notice family is now live as a pure `src/engine/field-notices.ts` seam: filed-route and notebook-ready title/text/variant derivation moved out of `game.ts`, while timer mutation, enqueue ordering, guided notice priority, and save writes stay in the runtime controller.
- The next controller-protection follow-on is now live as a pure `src/engine/field-season-wrap.ts` seam: station-strip wrap and subtitle composition moved out of `field-season-board.ts`, while route-state builders, replay-note generation, atlas, archive, expedition, and save-facing progression logic stay in the board resolver.
...
- Packet `158` is complete: the RC/review-drop handoff is clean, the queue and packet truth are reconciled, the High Pass rime-footing seam is green, `alpha:rc` is the strict release gate, and packet `159` is now the active playtest-kit step.
- The post-alpha path remains feedback-first: do not open season three, biome six, a broader planner, crafting, combat, inventory, or direct API field-guide mode during packet `159`; packet `159` should prepare playtest materials, packet `160` should triage feedback, and only then should the seeded Source to Shore beta campaign be promoted.
- For the current two-lane push, lane boundaries are strict: lane 1 owns playability, systems, station, routes, support, replay, and integration proof; lane 2 owns world richness, science content, atlas/journal payoff, close-look, sketchbook, spatial readability, vertical traversal, and sub-ecosystem depth. Former lane 3 work routes through lane 2, former lane 4 work routes through lane 1, and shared files require an explicit handoff note before another lane edits them.
- Packet `182` is complete as an explicit blocker handoff, not a clean visual signoff: validation, science, tests, build, alpha RC, and browser console proof passed, but native `256x160` station/title/journal readability still blocks breadth. Packet `192` must start with lane 1's handheld readability scout/repair/review chain, and lane 2 breadth stays parked until `ECO-20260514-critic-01` clears that gate.
- During packets `130`-`157`, each lane advances on its own scout -> main -> critic chain; cross-lane blockers should be rare and must be recorded in the queue instead of resolved by silent overlapping edits.
...
- Source review drops should use `npm run review:pack` plus `npm run review:verify -- <archive>` instead of `npm pack`, because the verified archive must include `package-lock.json` and prove clean `npm ci`, agent validation, tests, and build from a fresh extract.
- Alpha review save snapshots now live as debug-only plain `SaveState` builders in `src/engine/debug-save-snapshots.ts`, exposed through `window.get_debug_save_snapshots()` for browser-console localStorage setup; future snapshot updates should keep them schema-neutral and covered by `src/test/save-snapshots.test.ts`.
- Test-only debug snapshot runtime helpers now live in `src/test/debug-snapshot-harness.ts`; future station/map/journal snapshot proof should reuse that helper instead of rebuilding local boot/menu helpers inside each test.
...
- Packet `159` is complete: `docs/alpha-playtest-kit.md` and `docs/playtest-comprehension-rubric.md` now provide the alpha playtest run kit, save setup, crash checklist, comprehension prompts, spatial/place-memory checks, and route/support observation protocol.
- Packet `160` closed by explicit product-owner override rather than real packet `159` session notes: `npm run alpha:rc` passed, no external child-session evidence was fabricated, and Source to Shore was selected as the next beta direction under that override.
