...
- The current active strategic priority is the packet `130`-`157` alpha runway mega-push: source-complete review hygiene, deterministic playthrough proof, homecoming and nursery payoff, five-biome tactile polish, route/support clarity, science/copy audit, maintainability extraction, external feedback integration, and alpha release-candidate packaging.
- The alpha runway is deliberately feedback-first: do not open season three, biome six, a broader planner, crafting, combat, inventory, or direct API field-guide mode unless the packet `157` post-alpha scope gate records evidence that the completed arc is stable enough.
...
- Packet `130` should run first in the alpha runway because source-complete review-drop hygiene is the gate that keeps outside feedback useful and reproducible.
- Later packets may touch Beach, Forest, Treeline, Tundra, homecoming, nursery, route support, and maintainability, but they should do so as alpha hardening inside the current five-biome arc rather than as season-three or biome-six expansion.
