...
describe('runtime smoke loop', () => {
  it('covers title, play, inspect, menu, world-map travel, journal, and reload persistence', async () => {
    ...
    seedSave.completedFieldRequestIds = ['beach-shore-shelter'];
    persistSave(seedSave);
  ...
  it('keeps unsupported fullscreen toggles save-safe and lets Escape recover from the menu', () => {
    ...
    const seedSave = createNewSaveState('runtime-unsupported-fullscreen-seed');
    seedSave.settings.fullscreen = true;
    persistSave(seedSave);

    const canvas = document.createElement('canvas') as unknown as HTMLCanvasElement;
    createGame(canvas, seedSave);

    tapKey(fakeWindow, 'm');
    let state = readState(fakeWindow);
    expect(state.mode).toBe('menu');
    expect(state.menu?.returnMode).toBe('title');
    expect(state.menu?.selectedAction).toBe('toggle-fullscreen');
    expect(state.settings.fullscreen).toBe(true);

    tapKey(fakeWindow, 'Enter');
    state = readState(fakeWindow);
    expect(state.mode).toBe('menu');
    expect(state.settings.fullscreen).toBe(false);
    expect(loadOrCreateSave().settings.fullscreen).toBe(false);
  ...
  it('turns the beach start into a notebook-ready Shore Shelter outing and files it at the station', () => {
    ...
    expect(state.activeFieldRequest?.routeV2).toMatchObject({
      status: 'gathering',
    ...
    expect(state.activeFieldRequest).toMatchObject({
      id: 'beach-shore-shelter',
      progressLabel: 'Return To Lee Pocket',
    });
    expect(state.activeFieldRequest?.routeV2).toMatchObject({
      status: 'gathering',
      selectedSupportId: 'note-tabs',
    ...
    expect(state.activeFieldRequest).toMatchObject({
      id: 'beach-shore-shelter',
      progressLabel: 'Return To Tide Line',
    });
    expect(state.activeFieldRequest?.routeV2).toMatchObject({
      status: 'gathering',
      selectedSupportId: 'note-tabs',
    ...
    expect(state.fieldRequestNotice).toMatchObject({
      title: 'NOTEBOOK READY',
      text: 'Use M -> Field station, then Enter to file the Shore Shelter note.',
    ...
    expect(state.fieldStation?.routeBoard).toMatchObject({
      ...
      notebookReady: {
        requestId: 'beach-shore-shelter',
        previewLabel: 'SHORE SHELTER',
        previewText: 'American Dunegrass, Driftwood, and Bull Kelp Wrack mark shelter from dune edge to tide line.',
      },
      beats: [
    ...
    expect(state.fieldStation?.seasonWrap).toMatchObject({
      label: 'SHORE SHELTER',
      text: 'American Dunegrass, Driftwood, and Bull Kelp Wrack mark shelter from dune edge to tide line.',
    });
  ...
  it('lets Wrack Shelter finish through beach-hopper during the active wrack-hold window', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-wrack-opportunity-seed');
    seededSave.selectedOutingSupportId = 'note-tabs';
    ...
    state = advanceWhileHoldingKeyUntil(
      ...
      (nextState) =>
        nextState.zoneId === 'tide-line'
        && (nextState.player?.x ?? 0) >= 440
        && nextState.nearbyInspectables.some((entity: any) => entity.entryId === 'beach-hopper'),
      620,
    );
    const beachHopper = state.nearbyInspectables.find((entity: any) => entity.entryId === 'beach-hopper');
    if (!beachHopper) {
      throw new Error('Expected beach hopper along the tide line during wrack hold.');
    }

    game.inspectEntity(beachHopper.entityId);
    fakeWindow.advanceTime?.(16);
    state = readState(fakeWindow);
    expect(seededSave.routeV2Progress).toMatchObject({
      requestId: 'beach-shore-shelter',
    ...
    expect(state.fieldStation?.seasonWrap).toMatchObject({
      label: 'SHORE SHELTER',
      text: 'American Dunegrass, Driftwood, and Beach Hopper mark shelter from dune edge to tide line.',
    });
  ...
  it('does not let a lee-pocket beach-hopper finish Wrack Shelter after crossing into tide-line', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('probe-2');
    ...
    let state = advanceWhileHoldingKeyUntil(
      ...
      (nextState) =>
        nextState.zoneId === 'tide-line'
        && (nextState.player?.x ?? 0) >= 400
        && nextState.nearbyInspectables.some((entity: any) =>
          entity.entryId === 'beach-hopper' && entity.x === 385,
        ),
      620,
    );

    const boundaryHopper = state.nearbyInspectables.find((entity: any) =>
      entity.entryId === 'beach-hopper' && entity.x === 385,
    );
    if (!boundaryHopper) {
      throw new Error('Expected the seeded lee-pocket beach hopper near the tide-line boundary.');
    }

    game.inspectEntity(boundaryHopper.entityId);
    fakeWindow.advanceTime?.(16);
    state = readState(fakeWindow);
  ...
  it('lets hand lens prefer the wrack clue over a nearer tide-line decoy on the last Shore Shelter stage', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-hand-lens-wrack-targeting-seed');
    seededSave.selectedOutingSupportId = 'hand-lens';
    seededSave.routeV2Progress = {
      requestId: 'beach-shore-shelter',
      status: 'gathering',
    ...
    let state = advanceWhileHoldingKeyUntil(
      ...
      (nextState) =>
        nextState.zoneId === 'tide-line'
        && (nextState.player?.x ?? 0) >= 502
        && (nextState.player?.x ?? 999) <= 506
        && nextState.nearbyInspectables.some((entity: any) => entity.entryId === 'bull-kelp-wrack' && entity.x === 486)
  ...
  it('keeps non-hand-lens supports on the normal nearest tide-line inspectable in the same wrack setup', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-note-tabs-wrack-decoy-seed');
    seededSave.selectedOutingSupportId = 'note-tabs';
    seededSave.routeV2Progress = {
      requestId: 'beach-shore-shelter',
      status: 'gathering',
    ...
    tapKey(fakeWindow, 'e');
    state = readState(fakeWindow);
    expect(state.openBubble?.entryId).not.toBe('bull-kelp-wrack');
    expect(seededSave.routeV2Progress).toMatchObject({
      requestId: 'beach-shore-shelter',
      status: 'gathering',
      evidenceSlots: [
  ...
  it('turns the coastal front-half transition into a notebook-ready Open To Shelter outing and files it at the station', () => {
    ...
    expect(state.fieldStation?.seasonWrap).toEqual({
      ...
      text: 'Yellow Sand Verbena, Shore Pine, and Nurse Log mark open coast meeting forest-edge shelter.',
    });
  ...
  it('keeps Forest Survey on the default recorded notice style', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-forest-survey-notice-seed');
    seededSave.completedFieldRequestIds = ['beach-shore-shelter', 'forest-hidden-hollow', 'forest-moisture-holders'];
    ...
    const state = readState(fakeWindow);
    expect(seededSave.completedFieldRequestIds).toContain('forest-survey-slice');
    expect(state.fieldRequestNotice).toMatchObject({
      title: 'TASK RECORDED',
      text: 'Forest Survey',
      variant: 'default',
  ...
  it('lets the player reach the new opening dune shoulder before the crest', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-dune-shoulder-seed');
    persistSave(seededSave);
    ...
    advanceWhileHoldingKeyUntil(
      ...
      'ArrowRight',
      (nextState) =>
        (nextState.player?.x ?? 0) >= 138 &&
        (nextState.player?.x ?? 999) <= 182 &&
        nextState.nearbyDoor?.inRange === false &&
        (nextState.nearbyTravelTarget ?? null) === null &&
        nextState.nearbyInspectables.some((entity: any) =>
          ['beach-grass', 'beach-pea', 'sand-verbena'].includes(entity.entryId),
        ),
      320,
    );

    const shoulderState = readState(fakeWindow);

    expect(['dune-edge', 'dry-sand']).toContain(shoulderState.zoneId);
    expect(shoulderState.nearbyDoor?.inRange).toBe(false);
    expect(shoulderState.nearbyTravelTarget ?? null).toBeNull();
    expect(
      shoulderState.nearbyInspectables.some((entity: any) =>
        ['beach-grass', 'beach-pea', 'sand-verbena'].includes(entity.entryId),
      ),
    ).toBe(true);

    holdKey(fakeWindow, 'ArrowRight', 70);
    const recoveredState = readState(fakeWindow);
    expect(recoveredState.zoneId).toBe('dry-sand');
    expect(recoveredState.player?.x).toBeGreaterThan(shoulderState.player?.x ?? 0);
  });
  ...
  it('turns the treeline threshold into a last-tree shelter followed by the existing lee pocket', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-treeline-last-tree-shelter-seed');
    persistSave(seededSave);
    ...
    let state = advanceWhileHoldingKeyUntil(
      ...
      'ArrowRight',
      (nextState) =>
        nextState.zoneId === 'krummholz-belt' &&
        Math.abs(nextState.player?.vy ?? 999) <= 1 &&
        (nextState.player?.x ?? 0) >= 224 &&
        (nextState.player?.x ?? 999) <= 250 &&
        (nextState.player?.y ?? 0) >= 98 &&
        (nextState.player?.y ?? 999) <= 106 &&
        nextState.nearbyInspectables.some((entity: any) => entity.entryId === 'krummholz-spruce'),
      420,
    );

    expect(state.zoneId).toBe('krummholz-belt');
    expect(state.player?.x).toBeGreaterThanOrEqual(224);
    expect(state.player?.x).toBeLessThanOrEqual(250);
    expect(state.player?.y).toBeGreaterThanOrEqual(98);
    expect(state.player?.y).toBeLessThanOrEqual(106);
    expect(
      state.nearbyInspectables.some((entity: any) => entity.entryId === 'krummholz-spruce'),
    ).toBe(true);
    expect(
      state.nearbyInspectables.some((entity: any) => entity.entryId === 'bunchberry'),
    ).toBe(true);
    ...
    expect(state.zoneId).toBe('dwarf-shrub');
    expect(state.player?.x).toBeGreaterThanOrEqual(398);
  ...
  it('adds one compact snow-meadow drift hold before a shorter thaw-skirt approach catch', () => {
    ...
    let state = advanceWhileHoldingKeyUntil(
      ...
      'ArrowRight',
      (nextState) =>
        nextState.zoneId === 'snow-meadow' &&
        (nextState.player?.x ?? 0) >= 152 &&
        (nextState.player?.x ?? 999) <= 180 &&
        (nextState.player?.y ?? 0) >= 99 &&
        (nextState.player?.y ?? 999) <= 103 &&
        nextState.nearbyInspectables.some((entity: any) =>
          ['frost-heave-hummock', 'reindeer-lichen', 'moss-campion'].includes(entity.entryId),
        ),
      220,
    );

    expect(state.zoneId).toBe('snow-meadow');
    expect(state.player?.x).toBeGreaterThanOrEqual(152);
    expect(state.player?.x).toBeLessThanOrEqual(180);
    expect(state.player?.y).toBeGreaterThanOrEqual(99);
    expect(state.player?.y).toBeLessThanOrEqual(103);
    expect(
      state.nearbyInspectables.some((entity: any) => entity.entryId === 'frost-heave-hummock'),
    ).toBe(true);
    expect(
      state.nearbyInspectables.some((entity: any) => entity.entryId === 'reindeer-lichen'),
    ).toBe(true);
    ...
    state = advanceWhileHoldingKeyUntil(
      ...
      (nextState) =>
        nextState.zoneId === 'snow-meadow' &&
        Math.abs(nextState.player?.vy ?? 999) <= 1 &&
        (nextState.player?.x ?? 0) >= 248 &&
        (nextState.player?.x ?? 999) <= 282 &&
        (nextState.player?.y ?? 0) >= 95 &&
        (nextState.player?.y ?? 999) <= 104 &&
    ...
    expect(state.zoneId).toBe('snow-meadow');
    expect(state.player?.x).toBeGreaterThanOrEqual(248);
    expect(state.player?.x).toBeLessThanOrEqual(282);
    expect(state.player?.y).toBeGreaterThanOrEqual(95);
    expect(state.player?.y).toBeLessThanOrEqual(104);
    expect(
      state.nearbyInspectables.some((entity: any) => entity.entryId === 'bigelows-sedge'),
    ).toBe(true);
    expect(
      state.nearbyInspectables.some((entity: any) => entity.entryId === 'white-tailed-ptarmigan'),
    ).toBe(true);
  ...
  it('opens the world-map field station, claims field credit, and buys trail stride', () => {
    ...
    expect(state.fieldStation?.arrivalPulse).toBe(0);
    expect(state.fieldStation?.routeBoard).toMatchObject({
  ...
  it('exposes homecoming copy state only for the earned station return', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-field-station-homecoming-copy');
    seededSave.completedFieldRequestIds = ['coastal-edge-moisture'];
    persistSave(seededSave);
    ...
    expect(state.mode).toBe('field-station');
    expect(state.fieldStation?.arrivalMode).toBe('homecoming');
    expect(state.fieldStation?.homecoming).toEqual({
      label: 'WELCOME BACK',
    ...
    expect(state.mode).toBe('field-station');
    expect(state.fieldStation?.arrivalMode).toBe('default');
    expect(state.fieldStation?.homecoming).toBeNull();
    expect(state.fieldStation?.backdropAccent).toMatchObject({
  ...
  it('clears stale station guidance notices when an earned homecoming station open begins', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-field-station-homecoming-notice-cleanup');
    for (const entryId of ['banana-slug', 'sword-fern', 'redwood-sorrel', 'fir-cone']) {
      recordDiscovery(seededSave, forestBiome.entries[entryId], 'forest');
    }
    seededSave.completedFieldRequestIds = ['forest-hidden-hollow', 'forest-moisture-holders'];
  ...
  it('opens the world map on the outing target when route marker is already selected', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-route-marker-open-focus-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
    ...
    seededSave.purchasedUpgradeIds = ['trail-stride', 'field-step', 'route-marker'];
    seededSave.selectedOutingSupportId = 'route-marker';
    persistSave(seededSave);
    ...
    selectMenuAction(fakeWindow, 'world-map');
    tapKey(fakeWindow, 'Enter');
    const state = advanceUntil(fakeWindow, (nextState) => nextState.scene === 'world-map');
    expect(state.worldMap?.currentLocationId).toBe('forest');
    expect(state.worldMap?.focusedLocationId).toBe('coastal-scrub');
    expect(state.worldMap?.originLabel).toBe('FROM FOREST TRAIL');
    expect(state.worldMap?.routeMarkerLocationId).toBe('coastal-scrub');
    expect(state.worldMap?.routeReplayLabel).toBe('Today: Haze Shift');
  });
  ...
  it('falls back to hand-lens when a save still carries a locked route-marker support id', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-locked-route-marker-support-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
  ...
  it('switches the route board to tundra and can hand the outing guide to route marker', () => {
    ...
    expect(state.fieldStation?.selectedOutingSupportLabel).toBe('PLACE TAB');
    expect(state.fieldStation?.seasonWrap).toEqual({
      label: 'TODAY',
  ...
  it('switches the route board to coastal scrub and can hand the outing guide to route marker', () => {
    ...
    expect(state.fieldStation?.seasonWrap).toEqual({
      label: 'TODAY',
      text: 'Where does wind-slowed sand start feeling calmer?',
    });
  ...
  it('shows the held-sand route replay note when re-entering coastal scrub during the active process window', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-held-sand-route-replay-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
  ...
  it('keeps note tabs on the stable scrub pattern line during the Held Sand replay window', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-held-sand-note-tabs-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
  ...
  it('files High Pass from the live talus-hold loop and settles the completed field arc', () => {
    ...
    try {
      ...
      expect(readyPocketState.activeFieldRequest?.routeV2).toMatchObject({
        status: 'gathering',
      ...
      expect(nearest).toMatchObject({ entryId: 'talus-cushion-pocket' });
      expect(readyPocketState.fieldRequestHint).toMatchObject({
        label: 'NOTEBOOK J',
        title: 'High Pass',
        variant: 'support-biased',
      });
      ...
      tapKey(fakeWindow, 'Escape');
      state = readState(fakeWindow);
      const renderedState = fakeWindow.render_game_to_text?.() ?? '';
      expect(state.scene).toBe('world-map');
      expect(state.fieldRequestNotice).toMatchObject({
        title: 'HIGH PASS',
        variant: 'filed-route',
  ...
  it('keeps non-hand-lens supports on the nearer open-fell inspectable in the same High Pass pocket setup', () => {
    ...
    try {
      ...
      expect(beforeInspectState.activeFieldRequest?.routeV2).toMatchObject({
        status: 'gathering',
        selectedSupportId: 'note-tabs',
        evidenceSlots: [
          { slotId: 'stone-lift', entryId: 'frost-heave-boulder' },
  ...
  it('keeps field-request notice timers paused while the menu hides feedback', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-field-request-notice-menu-pause-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
  ...
  it('opens first field-season guidance world-map focus on the next habitat', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-guided-map-focus-seed');
    seededSave.completedFieldRequestIds = ['beach-shore-shelter'];
    persistSave(seededSave);
  ...
  it('shows the tundra survey place-tab question once the inland line reaches Tundra Survey', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-place-tab-tundra-survey-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
    ...
    expect(state.fieldStation?.routeBoard).toMatchObject({
      ...
      beats: [
        ...
        { id: 'tundra-short-season', status: 'done' },
  ...
  it('keeps the tundra survey place-tab question when the beat reframes as Bright Survey', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-place-tab-bright-survey-seed');
    seededSave.completedFieldRequestIds = [
      'forest-hidden-hollow',
      'forest-moisture-holders',
      'forest-survey-slice',
    ...
    expect(state.fieldStation?.routeBoard).toMatchObject({
      ...
      beats: [
        { id: 'treeline-shelter', status: 'done' },
        { id: 'tundra-short-season', status: 'done' },
        { id: 'tundra-survey', status: 'active', title: 'Bright Survey' },
      ],
    });
  ...
  it('turns the coastal-scrub bluff into one optional lookout and keeps the low route recoverable', () => {
    const { window: fakeWindow, document } = installFakeDom();
    ...
    try {
      ...
      for (let index = 0; index < 90; index += 1) {
        ...
        if (
          Math.abs(nextState.player?.vy ?? 999) <= 1 &&
          (nextState.player?.x ?? 0) >= 324 &&
          (nextState.player?.x ?? 999) <= 350 &&
          (nextState.player?.y ?? 0) >= 78 &&
          (nextState.player?.y ?? 999) <= 94 &&
          nextState.nearbyInspectables.some((entity: any) =>
            ['pacific-wax-myrtle', 'song-sparrow', 'coyote-brush', 'deer-mouse'].includes(entity.entryId),
          )
        ) {
          foundState = nextState;
          break;
      ...
      if (!foundState) {
        throw new Error(`coastal-bluff-lookout ${JSON.stringify(readState(fakeWindow))}`);
      }
      state = foundState;
      expect(state.zoneId).toBe('windbreak-swale');
      expect(state.habitatChipLabel).toBe('Windbreak Swale');
  ...
  it('builds the coastal-scrub family from the normal entry into the bluff lookout and back to the low route', () => {
    const { window: fakeWindow, document } = installFakeDom();
    ...
    if (!foundState) {
      throw new Error(`coastal-family-lookout ${JSON.stringify(readState(fakeWindow))}`);
    }
    state = foundState;
    expect(state.zoneId).toBe('windbreak-swale');
  ...
  it('adds a tucked middle swale pocket and still releases cleanly into the pine rest', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-coastal-signature-pocket-seed');
    persistSave(seededSave);
    ...
    const pocketState = advanceWhileHoldingKeyUntil(
      fakeWindow,
      'ArrowRight',
      (nextState) =>
        nextState.zoneId === 'windbreak-swale' &&
        (nextState.player?.x ?? 0) >= 350 &&
        (nextState.player?.x ?? 999) <= 376 &&
        (nextState.player?.y ?? 0) >= 109 &&
        (nextState.player?.y ?? 999) <= 116 &&
        Math.abs(nextState.player?.vy ?? 999) <= 1 &&
        nextState.nearbyInspectables.some((entity: any) =>
          ['beach-strawberry', 'song-sparrow', 'pacific-wax-myrtle', 'deer-mouse'].includes(entity.entryId),
        ),
      560,
    );

    expect(pocketState.zoneId).toBe('windbreak-swale');
    expect(pocketState.habitatChipLabel).toBe('Windbreak Swale');
    expect(
      pocketState.nearbyInspectables.some((entity: any) =>
        ['beach-strawberry', 'song-sparrow', 'pacific-wax-myrtle', 'deer-mouse'].includes(entity.entryId),
      ),
    ).toBe(true);
    expect(pocketState.nearbyTravelTarget ?? null).toBeNull();
    expect(pocketState.nearbyDoor?.inRange ?? false).toBe(false);

    const pineRestState = advanceWhileHoldingKeyUntil(
      fakeWindow,
      'ArrowRight',
      (nextState) =>
        nextState.zoneId === 'shore-pine-stand' &&
    ...
    expect(pineRestState.zoneId).toBe('shore-pine-stand');
    expect(pineRestState.player?.x).toBeGreaterThan(pocketState.player?.x ?? 0);
    expect(
      pineRestState.nearbyInspectables.some((entity: any) =>
        ['kinnikinnick', 'song-sparrow', 'shore-pine'].includes(entity.entryId),
      ),
    ).toBe(true);
  });
  ...
  it('keeps treeline-tundra corridor threshold pacing from advancing visits before exit', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-treeline-tundra-corridor-accounting-seed');
    seededSave.lastBiomeId = 'treeline';
    seededSave.worldStep = 2;
    seededSave.biomeVisits.treeline = 1;
    seededSave.biomeVisits.tundra = 0;
    persistSave(seededSave);
    ...
    const worldStepBeforeCorridor = seededSave.worldStep;
    const lastBiomeBeforeCorridor = seededSave.lastBiomeId;
    const treelineVisitsBeforeCorridor = seededSave.biomeVisits.treeline ?? 0;
    const tundraVisitsBeforeCorridor = seededSave.biomeVisits.tundra ?? 0;
    ...
    expect(state.worldState.worldAge).toBe(worldStepBeforeCorridor);
    expect(state.worldState.weather).toBe('ridge-wind');
    ...
    expect(state.corridor?.zoneId).toBe('wind-bluff');
    expect(state.worldState.worldAge).toBe(worldStepBeforeCorridor);
    expect(state.worldState.weather).toBe('light-flurry');
    expect(seededSave.worldStep).toBe(worldStepBeforeCorridor);
    expect(seededSave.lastBiomeId).toBe(lastBiomeBeforeCorridor);
    expect(seededSave.biomeVisits.treeline ?? 0).toBe(treelineVisitsBeforeCorridor);
    expect(seededSave.biomeVisits.tundra ?? 0).toBe(tundraVisitsBeforeCorridor);
    ...
    state = advanceWhileHoldingKeyUntil(
      ...
      (nextState) =>
        nextState.sceneBiomeId === 'treeline-tundra-corridor' &&
        nextState.corridor?.ownerBiomeId === 'treeline',
      240,
    );
    expect(state.corridor?.zoneId).toBe('lichen-fell');
    expect(state.worldState.worldAge).toBe(worldStepBeforeCorridor);
    ...
    expect(state.corridor).toBeNull();
    expect(seededSave.worldStep).toBe(worldStepBeforeCorridor + 1);
    expect(seededSave.lastBiomeId).toBe('tundra');
    expect(seededSave.biomeVisits.treeline ?? 0).toBe(treelineVisitsBeforeCorridor);
  ...
  it('surfaces the new dune-lupine comparison once the preferred scrub note is unlocked', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-dune-lupine-comparison-seed');
    recordDiscovery(seededSave, beachBiome.entries['dune-lupine'], 'beach');
    recordDiscovery(seededSave, coastalScrubBiome.entries['dune-lupine'], 'coastal-scrub');
    recordDiscovery(seededSave, beachBiome.entries['sand-verbena'], 'beach');
    recordDiscovery(seededSave, coastalScrubBiome.entries['pacific-wax-myrtle'], 'coastal-scrub');
    recordDiscovery(seededSave, coastalScrubBiome.entries['coyote-brush'], 'coastal-scrub');
    persistSave(seededSave);
    ...
    let state = readState(fakeWindow);
    expect(state.journal?.selectedBiomeId).toBe('beach');
    expect(state.journal?.selectedEntryId).toBe('dune-lupine');
    expect(state.journal?.comparison?.available).toBe(true);
    expect(state.journal?.comparison?.open).toBe(false);
    expect(state.journal?.comparison?.cards.map((card: any) => card.biomeId)).toEqual(['beach', 'coastal-scrub']);
  ...
  it('surfaces the new reindeer-lichen comparison once treeline and tundra note context are both unlocked', () => {
    const { window: fakeWindow, document } = installFakeDom();
    const seededSave = createNewSaveState('runtime-reindeer-lichen-comparison-seed');
    recordDiscovery(seededSave, treelineBiome.entries['reindeer-lichen'], 'treeline');
    recordDiscovery(seededSave, tundraBiome.entries['reindeer-lichen'], 'tundra');
    recordDiscovery(seededSave, treelineBiome.entries['arctic-willow'], 'treeline');
    recordDiscovery(seededSave, tundraBiome.entries['moss-campion'], 'tundra');
    persistSave(seededSave);
    ...
    expect(state.journal?.selectedEntryId).toBe('moss-campion');
    expect(state.journal?.comparison?.available).toBe(false);
    expect(state.journal?.comparison?.open).toBe(false);

    tapKey(fakeWindow, 'ArrowDown');
    state = readState(fakeWindow);
    expect(state.journal?.selectedEntryId).toBe('reindeer-lichen');
    expect(state.journal?.comparison?.available).toBe(true);
    expect(state.journal?.comparison?.cards.map((card: any) => card.biomeId)).toEqual(['treeline', 'tundra']);
  ...
  it('surfaces the new High Pass rime-footing note in the live open-fell chapter pocket', () => {
    ...
    try {
      ...
      expect(state.journal?.ecosystemNote).toEqual({
        ...
        summary: 'Rime favors low life on exposed High Pass ground, while tiny rock pockets give footholds.',
      });
      expect(state.journal?.observationPrompt).toMatchObject({
        family: 'neighbors',
