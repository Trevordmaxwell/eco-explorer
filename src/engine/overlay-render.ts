...
export function drawFieldStationOverlay({
  ...
  context.fillRect(0, 0, width, height);
  fillLeafGreenPanel(context, panelRect.x, panelRect.y, panelRect.w, panelRect.h);
  drawFieldStationHomecomingShell({
    context,
    panelRect,
    contentRect,
    palette,
    input: homecomingShellInput,
    arrivalPulse,
    arrivalMode,
  });
  ...
  drawFieldStationNurseryPage({
    context,
    palette,
    pageRect: makeRect(contentRect.x, bodyTop, contentRect.w, contentRect.y + contentRect.h - bodyTop),
    selectedNurseryCardId,
    nursery,
  });
}
