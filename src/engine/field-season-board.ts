...
import { applyFieldSeasonReplayNote } from './field-season-replay-notes';
import {
  FOREST_EXPEDITION_CHAPTER_REQUEST_ID,
  FOREST_EXPEDITION_EVIDENCE_TOTAL,
  NEXT_FIELD_SEASON_TARGET_BIOME_ID,
  getExpeditionEvidenceCount,
  isExpeditionNotebookReady,
...
function getForestStudyBeat(save: SaveState, forestSurveyLogged: boolean): Omit<FieldSeasonBoardBeat, 'status'> {
  if (!hasCompletedRequest(save, 'beach-shore-shelter')) {
    return {
      id: 'forest-study',
      title: 'Shore Shelter',
      detail: 'Inspect dune grass, lee cover, and wrack line from Dune Edge to Tide Line.',
    };
  }
...
export function resolveFieldSeasonBoardState(
  ...
  if (hasCompletedRequest(save, 'tundra-survey-slice')) {
    ...
    return applyFieldSeasonReplayNote(
